...
@section('content')
    ...
    @php
        ...
        $mostActive = $executives->sortByDesc('current_score')->first();
    ...
    <div class="exec-spotlight-row mb-4">
        <div class="exec-spotlight-card spotlight-green">
            ...
            @if($topScorer)
                <div class="spotlight-row">
                    ...
                    <div class="spotlight-stat text-success">+{{ $topScorer->current_score }} pts</div>
                </div>
            @else
        ...
        <div class="exec-spotlight-card spotlight-red">
            ...
            @if($needsAttention && $needsAttention->current_score < 0)
                <div class="spotlight-row">
                    <div class="spotlight-left">
                        <div class="spotlight-avatar" style="background: linear-gradient(135deg,#ef4444,#7f1d1d);">
                            {{ strtoupper(substr($needsAttention->name, 0, 2)) }}</div>
                        <div>
                            <div class="spotlight-name">{{ $needsAttention->name }}</div>
                            <div class="spotlight-meta">{{ $needsAttention->zone->name ?? '—' }}</div>
                        </div>
                    </div>
                    <div class="spotlight-stat text-danger">{{ $needsAttention->current_score }} pts</div>
                </div>
            @else
        ...
        <div class="d-flex align-items-center gap-2">
            <form action="{{ route('active_university.switch') }}" method="POST" id="global-univ-switcher" class="m-0">
                ...
                <div class="input-group input-group-sm">
                    ...
                    <select name="university_id" class="form-select bg-dark text-white border-secondary border-opacity-10"
            ...
            <form method="GET" action="{{ route('executives.index') }}" class="m-0">
                <div class="exec-search-wrap">
                    ...
                    <input type="text" name="search" class="exec-search-input" value="{{ request('search') }}"
                        placeholder="Search name, ID, email…">
                </div>
            </form>

            <button class="tims-table-control-btn" type="button" data-bs-toggle="modal" data-bs-target="#filterModal">
                    ...
                    @forelse($executives as $exec)
                        <tr class="exec-table-row">
                            ...
                            <td class="text-center">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="exec-table-avatar">{{ strtoupper(substr($exec->name, 0, 2)) }}</div>
                                    <div>
                            ...
                            {{-- Email --}}
                            <td class="text-center">
                                <span class="exec-email">{{ $exec->email }}</span>
                            </td>
                                ...
                                <div class="d-flex align-items-center gap-2 justify-content-center">
                                    @if($exec->university)
                                        @if($exec->university->logo_url)
                                            <img src="{{ $exec->university->logo_url }}" alt="{{ $exec->university->name }}"
                            ...
                            <td>
                                ...
                                <div class="exec-score-ring-wrap">
                                    <svg width="56" height="56" viewBox="0 0 56 56" style="transform:rotate(-90deg);">
                                        <circle cx="28" cy="28" r="{{ $radius }}" fill="none" stroke="{{ $trackColor }}" stroke-width="4"/>
                                        <circle cx="28" cy="28" r="{{ $radius }}" fill="none" stroke="{{ $color }}"
                            ...
                            {{-- Actions — plain button, menu teleported to <body> by JS --}}
                            <td class="text-end">
                                <button class="exec-action-btn exec-action-trigger"
                                    type="button"
                                    data-exec-id="{{ $exec->id }}">
                                    <i class="fa-solid fa-ellipsis-vertical"></i>
                                </button>

                                {{-- Menu template (hidden, lives inside table but never shown here) --}}
                                <ul class="exec-dropdown-tpl d-none" id="execMenu{{ $exec->id }}">
                                    <li>
                                    ...
                                    @can('manage_executives')
                                        ...
                                        <li>
                                            <form action="{{ route('executives.destroy', $exec->id) }}" method="POST" class="m-0"
    ...
    ══════════════════════════════════════════════ --}}
    <ul id="execFloatingMenu" class="exec-dropdown" style="
        display:none;
        position:fixed;
        z-index:99999;
        margin:0;
    ...
    <style>
        /* ── Spotlight row ── */
        .exec-spotlight-row { display:flex; flex-wrap:wrap; gap:12px; }
        @media(max-width:1200px){ .exec-spotlight-row{ justify-content:center; } }
        ...
        @media(max-width:1100px){ .exec-spotlight-card{ flex:1 1 calc(33.333% - 12px); } }
        @media(max-width:820px) { .exec-spotlight-card{ flex:1 1 calc(50% - 12px); } }
        @media(max-width:580px) { .exec-spotlight-card{ flex:1 1 100%; } }

        .exec-spotlight-card:hover { transform:translateY(-2px); box-shadow:0 8px 30px rgba(0,0,0,0.12); }

        .exec-spotlight-card::before {
            content:''; position:absolute; inset:0; border-radius:14px; opacity:.05; pointer-events:none;
        }
        ...
        .spotlight-eyebrow { font-size:9px; font-weight:700; letter-spacing:.09em; text-transform:uppercase; color:#9ca3af; }
        ...
        .spotlight-row  { display:flex; align-items:center; justify-content:space-between; gap:10px; margin-top:8px; }
        .spotlight-left { display:flex; align-items:center; gap:10px; }
        .spotlight-name { font-weight:700; color:#fff; font-size:13px; line-height:1.1; }
        .spotlight-meta { font-size:10.5px; color:#9ca3af; margin-top:1px; }
        .spotlight-stat { font-size:16px; font-weight:800; letter-spacing:-.02em; }
        .spotlight-big-num { font-size:28px; font-weight:800; letter-spacing:-.04em; color:#fff; }
        ...
        .exec-count-chip {
            ...
            padding:1px 10px; vertical-align:middle; margin-left:6px;
        }

        /* ── Search ── */
        .exec-search-wrap { position:relative; }
        .exec-search-icon { position:absolute; left:10px; top:50%; transform:translateY(-50%); color:#6b7280; font-size:11px; pointer-events:none; }
        .exec-search-input {
            background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08);
            border-radius:10px; color:#fff; font-size:12px;
            padding:6px 12px 6px 28px; width:200px;
            transition:border-color .2s,background .2s; outline:none;
        }
        .exec-search-input::placeholder { color:#4b5563; }
        .exec-search-input:focus { border-color:rgba(124,92,252,0.5); background:rgba(124,92,252,0.07); }

        /* ── Table ── */
        .exec-table thead th {
            background:rgba(255,255,255,0.025); color:#4b5563;
            font-size:10.5px; font-weight:700; letter-spacing:.07em;
            border-bottom:1px solid rgba(255,255,255,0.06);
            padding:11px 14px; white-space:nowrap;
        }
        .exec-table tbody td { padding:13px 14px; border-bottom:1px solid rgba(255,255,255,0.04); vertical-align:middle; }
        .exec-table-row:hover td { background:rgba(255,255,255,0.025); }
        .exec-table-row:last-child td { border-bottom:none; }

        .exec-emp-id-link {
            font-family:'Fira Mono','Courier New',monospace; font-size:12px; font-weight:600;
            color:#7c5cfc; text-decoration:none;
            background:rgba(124,92,252,0.1); border:1px solid rgba(124,92,252,0.2);
            padding:2px 9px; border-radius:6px; transition:background .15s;
        }
        ...
        .exec-zone-badge {
            ...
            background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08);
            border-radius:20px; font-size:11px; font-weight:600; color:#94a3b8;
        ...
        .exec-status-pill { display:inline-flex; align-items:center; gap:5px; font-size:11.5px; font-weight:600; padding:4px 11px; border-radius:20px; white-space:nowrap; }
        ...
        .exec-score-ring-wrap { position:relative; width:56px; height:56px; display:flex; align-items:center; justify-content:center; }
        .exec-score-ring-wrap svg { position:absolute; inset:0; }
        .exec-score-ring-label { position:relative; display:flex; flex-direction:column; align-items:center; justify-content:center; z-index:1; }
        ...
        .exec-expired-chip {
            ...
            letter-spacing:.06em; padding:1px 6px; margin-left:5px; vertical-align:middle;
        }
        ...
        .exec-action-btn.is-open { background:rgba(255,255,255,0.07); color:#e2e8f0; }
        ...
        #execFloatingMenu .dropdown-item {
            color:#94a3b8; font-size:13px; border-radius:8px;
            padding:8px 12px; transition:background .15s,color .15s;
            display:block; width:100%; text-align:left;
            background:none; border:none; cursor:pointer; text-decoration:none;
        }
        #execFloatingMenu .dropdown-item:hover { background:rgba(255,255,255,0.06); color:#f1f5f9; }
        #execFloatingMenu .dropdown-item.text-danger:hover { background:rgba(248,113,113,0.1); color:#f87171; }
        #execFloatingMenu .dropdown-divider { border-color:rgba(255,255,255,0.06); margin:4px 0; }
        ...
        .exec-modal-header .modal-title { font-size:15px !important; font-weight:700 !important; color:#f1f5f9 !important; letter-spacing:-0.01em; }
        .exec-modal-body   { padding:24px !important; }
        .exec-modal-footer { background:rgba(255,255,255,0.02); border-top:1px solid rgba(255,255,255,0.06) !important; padding:16px 24px !important; }

        .exec-info-card { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:12px; padding:14px 16px; height:100%; transition:border-color .15s; }
        .exec-info-card:hover { border-color:rgba(255,255,255,0.14); }
        .exec-info-card-label { font-size:9.5px; font-weight:700; text-transform:uppercase; letter-spacing:.09em; color:#4b5563; margin-bottom:7px; display:flex; align-items:center; gap:6px; }
        .exec-info-card-label i { font-size:11px; }
        .exec-info-card-value { font-size:13px; font-weight:600; color:#e2e8f0; line-height:1.3; }

        .exec-modal-avatar {
            width:68px; height:68px; border-radius:50%;
            background:linear-gradient(135deg,#7c5cfc,#4338ca);
            display:flex; align-items:center; justify-content:center;
            font-size:24px; font-weight:800; color:#fff; margin:0 auto;
        ...
        .exec-modal-content .form-select {
            ...
            padding:9px 13px !important; transition:border-color .2s,background .2s;
        }
        .exec-modal-content .form-control:focus,
        .exec-modal-content .form-select:focus {
            background:rgba(124,92,252,0.07) !important; border-color:rgba(124,92,252,0.5) !important;
            box-shadow:0 0 0 3px rgba(124,92,252,0.12) !important; color:#f1f5f9 !important;
        }
        ...
        .exec-modal-content .form-label { font-size:11px !important; font-weight:700 !important; text-transform:uppercase; letter-spacing:.06em; color:#6b7280 !important; margin-bottom:6px !important; }
    </style>
                ...
                <div class="modal-content" style="background:#0f1322;border:1px solid rgba(255,255,255,0.08);border-radius:16px;box-shadow:0 15px 50px rgba(0,0,0,0.8);">
                    ...
                    <div class="modal-body p-4">
                        <div class="text-center mb-4">
                            <div class="mx-auto mb-3" style="width:64px;height:64px;font-size:1.5rem;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--primary-2));display:flex;align-items:center;justify-content:center;color:white;font-weight:700;box-shadow:0 5px 15px rgba(139,92,246,0.3);">
                                ...
                                <div class="col-4">
                                    <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                        <div class="d-flex align-items-center gap-2 mb-2">
                                            ...
                                            <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">{{ $f['label'] }}</small>
                                        </div>
                            ...
                            <div class="col-4">
                                <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                    <div class="d-flex align-items-center gap-2 mb-2">
                                        ...
                                        <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">Status</small>
                                    </div>
                                    <div class="mt-1">
                                        @if($exec->status === 'active')
                            ...
                            <div class="col-4">
                                <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                    <div class="d-flex align-items-center gap-2 mb-2">
                                        ...
                                        <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">Tier</small>
                                    </div>
                                    <div class="mt-1">
                            ...
                            <div class="col-4">
                                <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                    <div class="d-flex align-items-center gap-2 mb-2">
                                        ...
                                        <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">Score</small>
                                    </div>
                                    <span class="fw-bold d-block mt-1" style="font-size:15px;{{ $exec->current_score >= 0 ? 'color:#10b981;' : 'color:#f87171;' }}">
                                ...
                                <div class="col-6">
                                    <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                        <div class="d-flex align-items-center gap-2 mb-2">
                                            ...
                                            <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">Probation End</small>
                                        </div>
                                        <span class="{{ $exec->probation_end_date->isPast() ? 'text-danger fw-bold' : 'text-white' }}" style="font-size:12.5px;">
                                            {{ $exec->probation_end_date->toDateString() }}
                                        </span>
                                        <small class="d-block mt-1 {{ $exec->probation_end_date->isPast() ? 'text-danger' : 'text-secondary' }}" style="font-size:11px;">
                                ...
                                <div class="col-6">
                                    <div class="p-3 rounded-3 h-100" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);">
                                        <div class="d-flex align-items-center gap-2 mb-2">
                                            ...
                                            <small class="text-secondary" style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;">Reporting Manager</small>
                                        </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content exec-modal-content">
                    <div class="modal-header exec-modal-header">
                        <h5 class="modal-title"><i class="fa-solid fa-user-plus me-2" style="color:#10b981;"></i>Add New Executive</h5>
    ...
    @foreach($executives as $exec)
        <div class="modal fade" id="editExecutiveModal{{ $exec->id }}" tabindex="-1">
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content exec-modal-content">
                    <div class="modal-header exec-modal-header">
                        <h5 class="modal-title"><i class="fa-solid fa-user-pen me-2" style="color:#fbbf24;"></i>Edit Executive</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
    ...
    <div class="modal fade" id="filterModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content exec-modal-content">
                <div class="modal-header exec-modal-header">
                    <h5 class="modal-title"><i class="fa-solid fa-sliders me-2" style="color:#22d3ee;"></i>Advanced Filters</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
    ...
    {{-- ══════════════════════════════════════════════
         TELEPORT DROPDOWN SCRIPT
    ══════════════════════════════════════════════ --}}
    <script>
    (function () {
        const floatingMenu = document.getElementById('execFloatingMenu');
        let activeBtn = null;

        // Position the floating menu next to the trigger button
        function positionMenu(btn) {
            const r = btn.getBoundingClientRect();
            const menuW = 180;
            const menuH = floatingMenu.offsetHeight || 160;
            const spaceBelow = window.innerHeight - r.bottom;

            floatingMenu.style.left = (r.right - menuW + window.scrollX) + 'px';

            if (spaceBelow < menuH + 8) {
                // Not enough room below — open upward
                floatingMenu.style.top = (r.top + window.scrollY - menuH - 4) + 'px';
            } else {
                floatingMenu.style.top = (r.bottom + window.scrollY + 4) + 'px';
            }
        }
        ...
        // Open for a specific exec
        function openMenu(btn) {
            const execId = btn.dataset.execId;
            const tpl    = document.getElementById('execMenu' + execId);
            if (!tpl) return;

            // Clone the template items into the floating menu
            floatingMenu.innerHTML = tpl.innerHTML;
            ...
            // Re-attach modal triggers inside the cloned items
            floatingMenu.querySelectorAll('[data-bs-toggle="modal"]').forEach(el => {
        ...
        document.addEventListener('click', function (e) {
            const btn = e.target.closest('.exec-action-trigger');
            if (btn) {
                e.stopPropagation();
